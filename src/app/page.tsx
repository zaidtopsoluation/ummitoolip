...
export default function Home() {
  ...
  useEffect(() => {
    if (currentView === "puzzle" && puzzleTiles.length === 0) {
      initPuzzle(6);
    }
  }, [currentView, puzzleTiles.length]);
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 w-full">
            ...
            >
              ...
              <div>
                ...
                <p className="text-sm text-slate-400 leading-relaxed">
                  Engage with Zaid's mind-bending puzzle challenges. Unravel complex patterns, decode messages, and unlock hidden secrets within the sanctuary walls.
                </p>
              </div>
  ...
  if (isUnlocked && currentView === "puzzle") {
    if (puzzleTiles.length === 0) {
      return (
        <div className="min-h-screen w-full flex flex-col items-center justify-center bg-[#07040d] text-slate-100 p-6">
          <span className="text-sm font-mono text-slate-400 animate-pulse">Initializing sliding matrix...</span>
        </div>
      );
    }
    const totalTiles = puzzleGridSize * puzzleGridSize;
    return (
      <div className="min-h-screen w-full flex flex-col justify-between bg-[#07040d] text-slate-100 p-6 relative overflow-hidden">
        {/* Animated background lights */}
        <div className="absolute top-1/4 left-1/4 w-80 h-80 rounded-full bg-pink-900/10 filter blur-[80px] animate-pulse pointer-events-none" />
        <div className="absolute bottom-1/4 right-1/4 w-80 h-80 rounded-full bg-indigo-900/10 filter blur-[80px] animate-pulse pointer-events-none" style={{ animationDelay: "2s" }} />

        {/* Floating background puzzle pieces */}
        {Array.from({ length: 8 }).map((_, i) => (
          <div
            key={i}
            className="absolute text-2xl opacity-5 pointer-events-none animate-pulse"
            style={{
        ...
        <header className="w-full max-w-7xl mx-auto px-6 py-5 flex items-center justify-between z-30">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold tracking-tight bg-gradient-to-r from-pink-200 via-white to-indigo-200 bg-clip-text text-transparent">
                Puzzle Chamber
              </h1>
              <p className="text-xs text-slate-400 font-mono mt-1">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="px-4 py-2 rounded-full text-xs font-semibold bg-white/10 hover:bg-white/20 text-white/90 backdrop-blur-md border border-white/10 transition-all duration-300"
            >
              🏠 Back to Menu
            </button>
            <button
        ...
        <main className="flex-1 w-full max-w-5xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-12 gap-8 items-center py-6 z-20">
          ...
          <div className="lg:col-span-5 space-y-6">
            ...
            {showOriginalPreview && (
              <div className="p-4 rounded-2xl glass-panel border border-white/10 flex flex-col items-center text-center animate-fade-in">
